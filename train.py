...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint, debug_from,
    ...
    while iteration < opt.iterations + 1:
        batch_indices = training_dataset.load_batch_indices(batch_size=batch_size)
        # print(batch_indices)
        batch_data = [training_dataset.viewpoint_stack[i] for i in batch_indices]

        # for batch_data in training_dataset.viewpoint_stack[batch_indices]:
        iteration += 1
...
if __name__ == "__main__":
    ...
    # print(f"Use device: {args.data_device}")
    # os.environ["CUDA_VISIBLE_DEVICES"] = str(args.data_device)
    safe_state(args.data_device, args.quiet)
